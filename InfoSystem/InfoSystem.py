...
def sub_menu_print_base():
    ...
    print_command = {'1': print_per, '2': print_pos,
                     '3': print_emp, '9': clear}
...
def sub_menu_query():
    def query_by_dep():
        ...
        IO_system.print_a_list_with_indexes(QueryManager.employees_from_department(TypeOfDepartments(
            IO_system.select_from_enum(TypeOfDepartments, ' Введите код отдела: ')), data_manager))

    def query_by_pos():
        print(' Выберите должность:')
        IO_system.print_a_list_with_indexes(QueryManager.employees_of_position(data_manager, TypeOfDepartments(
            IO_system.select_from_enum(TypeOfPositions, ' Введите код должности: '))))

    def query_by_lvl():
        print(' Выберите уровень:')
        IO_system.print_a_list_with_indexes(QueryManager.employees_of_level(data_manager, TypeOfLevels(
